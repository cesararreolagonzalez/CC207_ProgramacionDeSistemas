                        ...
                        try:
                            ...
                            if(bytesPorCalcular == '1'):
                                if(valor >= 0 and valor <= 255):
                                    ...
                                    print('\t\t    ERROR: El operando para Modo Inmediato de 8 bits esta fuera de rango (%d > 255)' %valor)
                            elif(bytesPorCalcular == '2'):
                                if(valor >= 0 and valor <= 65535):
                                    print('\t\t    Modo Inmediato de 16 bits, %s bytes' %totalBytes)
                                else:
                                    print('\t\t    ERROR: El operando para Modo Inmediato de 16 bits esta fuera de rango (%d > 65535)' %valor) 
                        except:
                            print('\t\t    ERROR: La sintaxis de un operando para Modo Inmediato es #<Base Numerica><Valor>')         
                    else:
                        print('\t\t    ERROR: Los caracteres validos para binario son 0 y 1')
                else:
                    print('\t\t    ERROR: La sintaxis de un operando para Modo Inmediato es #<Base Numerica><Valor>') 
                ...
                if(esValido):
                    ...
                    if(bytesPorCalcular == '1'):
                        if(valor >= 0 and valor <= 255):
                            ...
                            print('\t\t    ERROR: El operando para Modo Inmediato de 8 bits esta fuera de rango (%d > 255). Rango de 0 a 255' %valor)
                    elif(bytesPorCalcular == '2'):
                        if(valor >= 0 and valor <= 65535):
                            print('\t\t    Modo Inmediato de 16 bits, %s bytes' %totalBytes)
                        else:
                            print('\t\t    ERROR: El operando para Modo Inmediato de 16 bits esta fuera de rango (%d > 65535). Rango de 0 a 65535' %valor)       
                else:
                    print('\t\t    ERROR: Los caracteres validos para decimal son 0-9')
            elif(operando[1] == '@'):
                if(len(operando) > 2):
                    ...
                    if(esValido):
                        ...
                        print('\t\t    ERROR: Los caracteres validos para octal son 0-7')
                else:
                    print('\t\t    ERROR: La sintaxis de un operando para Modo Inmediato es #<Base Numerica><Valor>') 
            elif(operando[1] == '$'):
                if(len(operando) > 2):
                    ...
                    if(esValido):
                        ...
                        print('\t\t    ERROR: Los caracteres validos para hexadecimal son A-F, a-f y 0-9')
                else:
                    print('\t\t    ERROR: La sintaxis de un operando para Modo Inmediato es #<Base Numerica><Valor>') 
            else:
                print('\t\t    ERROR: La sintaxis de un operando para Modo Inmediato es #<Base Numerica><Valor>')
...
def validar_operando_directo_o_extendido(operando):
    ...
    if(operando != 'NULL'):
        if(operando[0] == '%'):
            if(len(operando) > 1):
                esValido = validar_binario(operando)
                if(esValido):
                    valor = (int(operando[1:], 2))  
                else:
                    print('\t\t    ERROR: Los caracteres validos para binario son 0 y 1')
            else:
                print('\t\t    ERROR: La sintaxis de Binario es %(0,1)<(0,1)>')
        elif(operando[0] >= '0' and operando[0] <= '9'):
            esValido = validar_decimal(operando)
            if(esValido):
                valor = (int(operando[0:])) 
            else:
                print('\t\t    ERROR: Los caracteres validos para decimal son 0-9')
        elif(operando[0] == '@'):
            if(len(operando) > 1):
                esValido = validar_octal(operando)
                if(esValido):
                    valor = (int(operando[1:], 8))
                else:
                    print('\t\t    ERROR: Los caracteres validos para octal son 0-7')
            else:
                print('\t\t    ERROR: La sintaxis de Octal es @(0-7)<(0-7)>')
        elif(operando[0] == '$'):
            if(len(operando) > 1):
                esValido = validar_hexadecimal(operando)
                if(esValido):
                    valor = (int(operando[1:], 16))
                else:
                    print('\t\t    ERROR: Los caracteres validos para hexadecimal son A-F, a-f y 0-9')
            else:
                print('\t\t    ERROR: La sintaxis de Hexadecimal es $(A-F, a-f, 0-9)<(A-F, a-f, 0-9)>')
        else:
            print('\t\t    ERROR: La sintaxis de un operando para Modo Directo es <Entero Positivo><Valor>')
    return valor
...
def identificar_indizado(operando, lista_direccionamientos):
    ...
    if('[' in operando or ']' in operando):
        ...
        if(valor_valido and valor == 'D' or valor == 'd'):
            ...
            if(not valor_valido):
                print('\t\t    ERROR DE VALOR: El rango de <Valor> para Modo Indizado Indirecto de 16 Bits es de 0 a 65,535')
            if(not registro_valido):
                print('\t\t    ERROR DE REGISTRO: Los Registros validos para Modo Indizado de 16 Bits son X, Y, PC y SP')
                if(error_sintaxis):
                    print('\t\t    ERROR DE SINTAXIS: La sintaxis de Modo Indizado Indirecto de 16 Bits  es [<Valor Decimal>,<Registro>]')
                    if('[' in error):
                        print('\t\t\t\t      Falta el corchete de apertura')
                    elif(']' in error):
                        print('\t\t\t\t       Falta el corchete de cierre')
            ...
            if(numero != None):
                ...
                if(valor[0] == '@' or valor[0] == '$' or valor[0] == '%' or valor[0] == '-'):
                    ...
                    if(registro in registros_computadora):
                        registro_valido = True
                    if(not valor_valido):
                        print('\t\t    ERROR DE VALOR: Los valores validos en Modo Indizado de Acumulador son A, B, D') 
                    if(not registro_valido):
                        print('\t\t    ERROR  DE REGISTRO: Los registros validos en Modo Indizado de Acumulador son X, Y, SP o PC') 
                    if(valor_valido and registro_valido):
                        informacion = lista_direccionamientos[3]
                        totalBytes = informacion[4]
