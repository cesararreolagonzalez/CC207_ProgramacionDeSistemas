...
####################################################################################################################
                                                ...
                                                PRÁCTICA #10
                        -----------------------------------------------------------------
                                        Arreola González, César Alejandro
                                                  (206547125)
...
def direccionamiento_particular_tabop(codop):
    ...
    print("** DIRECCIONAMIENTO **",direccionamiento)
    print("** INFORMACION **",lista_informacion)
    print("** TOTAL **",totalDireccionamiento)
...
def evaluando_partes_linea_ensamblador(etiqueta,codop,operando):
    ...
    global temporal_archivo_tabsimg
    global lista_direccionamientos
    ...
    ##########################################
    #--------------------------------------------------------------------------------------------------------------
    if(re.match('ORG', codop, re.IGNORECASE) and etiqueta != 'NULL'):
    ...
    #    DIRECTIVAS CONSTANTES de 1 byte
        ...
        if(operando[0] == '$' or operando[0] == '%' or operando[0] == '@' or (operando[0] >= '0' and operando[0] <= '9')):
            ...
            if(valor_operando >= 0 and valor_operando <= 255):
                resultado_evaluacion = 'Directiva de constante'
                bytesTotales = 1
            else:
    ...
    #    DIRECTIVAS DE RESERVA DE ESPACIO EN MEMORIA de 1 byte en 1 byte
        ...
        if(operando[0] == '$' or operando[0] == '%' or operando[0] == '@' or (operando[0] >= '0' and operando[0] <= '9')):
            ...
            if(valor_operando >= 0 and valor_operando <= 65535):
                ...
                resultado_evaluacion = 'Directiva de reserva'
            else:
                print('\t\t    ERROR DE RANGO: El operando de la directiva %s debe tener un rango de 0 a 65535'%codop.upper())
        else:
            print('\t\t    ERROR DE OPERANDO: El operando de la directiva %s debe estar representado en Decimal, Hexadecimal, Octal o Binario'%codop.upper()) 
...
'''####################################################################################################################
########################################    EVALUAR LINEAS DE LISTADO   ############################################
####################################################################################################################'''
        
        
def evaluar_lineas_de_listado():
    datosArchivoObjeto = []
    recalcularDireccion    = 0
    contador = 0
    existeORG = False
    tomarDireccion = True
    S1Completado = False
    ...
    print('-='*45)
    print('| VALOR  |   ETIQUETA   |   CODOP   |\t  OPERANDO\t\t|\tCODIGO MÁQUINA\t |')
    print('-='*45)
    ...
    for linea in archivo_listado: 
        ...
        codigo_maquina = calculoCodigoMaquina(direccionamiento, codop, operando, contador)
        if(codigo_maquina == ''): codigo_maquina = '-'
        longitudCodigoMaquina = len(codigo_maquina)   
        ####################################################################################################################
        
        impresion_valor = '|  %s  |' %valor
        impresion_etq = '     %s\t|' %etiqueta
        if(4 > len(codop)):
            impresion_codop = '   %s\t    |' %codop
        else:
            impresion_codop = '   %s    |'%codop
        ####################################################
        if(8 > len(operando)):
            impresion_operando = '  %s\t\t\t|' %operando
        elif(len(operando) > 8):
            impresion_operando = '  %s\t\t|' %operando
        else:
            impresion_operando = '  %s\t\t\t|' %operando
        ####################################################
        if(6 > len(codigo_maquina)):
            impresion_codMaq = ' %s\t\t\t |' %codigo_maquina
        elif(len(operando) > 8):
            impresion_codMaq = ' %s\t |' %codigo_maquina
        else:
            impresion_codMaq = ' %s\t\t |' %codigo_maquina
        
        print(impresion_valor+impresion_etq+impresion_codop+impresion_operando+impresion_codMaq)
        
        ####################################################################################################################
        
        if(codop.upper() == 'ORG' and not existeORG):
            existeORG = True
            
            rutaArchivo = os.path.abspath(nombre_archivo_asm)
            for caracter in rutaArchivo:
                datos += hex(ord(caracter))[2:].upper()
            datos += '0A'   
            
            
            longitud = format(int(str(len(datos) / 2).split('.')[0])+3, '02X')
            direccion = '0000'
            checksum = calcularCheckSum(longitud + direccion + datos)
            registroS0 +=  longitud + direccion + datos + checksum
            
            datosArchivoObjeto.append(registroS0)
            direccion = ''
            datos     = ''
            checksum  = ''
            longitud  = ''
        ...
        elif(direccionamiento.upper() == 'DIRECTIVA DE RESERVA' or direccionamiento.upper() == 'END' and not errores(codigo_maquina)):
            S1Completado = True
            if(direccionamiento.upper() == 'END'): print('-='*45)
                
        elif(direccionamiento.upper() != 'EQU' and direccionamiento.upper() != 'ORG' and not errores(codigo_maquina)):
            if(tomarDireccion):
                direccion = valor
                tomarDireccion = False
            while(len(codigo_maquina) > 0):
                datos += codigo_maquina[0:2]
                codigo_maquina = codigo_maquina[2:]
                
                if(len(datos) == 32):    
                    S1Completado = True
                    break
        
        if(S1Completado and len(datos) > 0 and not errores(codigo_maquina)):
            longitud = format(int(str(len(datos) / 2).split('.')[0])+3, '02X')
            checksum = calcularCheckSum(longitud + direccion + datos)
            
            registroS1 += longitud + direccion + datos + checksum
            datosArchivoObjeto.append(registroS1)
            ...
            if(len(codigo_maquina) > 0 and codigo_maquina != '-' and not errores(codigo_maquina)):
                if(tomarDireccion):
                    direccion = valor
                    tomarDireccion = False
                ...
                while(len(codigo_maquina) > 0):
                    ...
                    ##
                    if(len(datos) / 2 == 16): 
                        longitud = str(len(datos) / 2).split('.')[0]
                        longitud = hex(int(longitud)+3)[2:]
                        checksum = calcularCheckSum(longitud + direccion + datos)
                        ...
                        break
                    ##
                bytesAsignados = int(longitudCodigoMaquina/2) - contador_aux
                recalcularDireccion = int(direccion,16) + bytesAsignados
                direccion = format(recalcularDireccion, '04X')
        S1Completado = False
            
        ####################################################################################################################
        
    datosArchivoObjeto.append(registroS9)
    
    print('\n\nErrores:')
    print('-'*75)
    print('Error (1): El rango válido del desplazamiento es de -128 a 127')
    print('Error (2): El rango válido del desplazamiento es de -32768 a 32767')
    print('Error (3): La etiqueta del operando no existe en la tabla de símbolos')
    print('-'*75,'\n')
    print('Ruta del asm: '+os.path.abspath(nombre_archivo_asm))
    
    '''
    ####################################################################################################################
                                    Escribir los datos de objeto en el archivo
    ####################################################################################################################
    '''
    try:
        archivo_objeto = open(nombre_archivo_objeto, ANADIR_FINAL)
        contador_lineas = 0
        for linea in datosArchivoObjeto:
            if(contador_lineas+1 == len(datosArchivoObjeto)):
...
def errores(codigo_maquina):
    if(codigo_maquina == 'Error (1)' or codigo_maquina == 'Error (2)' or codigo_maquina == 'Error (3)'):
        return True
    return False

def calculoCodigoMaquina(info_direccionamiento, codop, operando, contador):
    ...
    if(direccionamiento.upper() == 'INHERENTE'):
    ...
    elif(direccionamiento.upper() == 'DIRECTO'):
        for direccionamientos in diccionario_codop[codop.upper()]:
    ...
    elif(direccionamiento.upper() == 'EXTENDIDO'):
        existeEnTabsim = False
        for direccionamientos in diccionario_codop[codop.upper()]:
            if(direccionamientos[2].upper() == 'EXT'):
                codigoMaquinaTabop =  direccionamientos[3]
                ###
                if(es_numerico(operando[1:])):
                    ...
                    for etiqueta in tabla_simbolos:
                        if(operando == etiqueta[0]):
                            valor_hexadecimal = etiqueta[1][:-1] #Si existe salto de linea en el valor 
                            existeEnTabsim = True
                            
                    if(not existeEnTabsim):
                        codigoMaquinaCalculado = 'Error (3)'
                        #print("\n\t\t    ERROR: El operando "+operando+" no existe en la tabla de símbolos")
                    else:
                        codigoMaquinaCalculado = codigoMaquinaTabop + valor_hexadecimal
                        
    elif(direccionamiento.upper() == 'INMEDIATO DE 8 BITS'):
        for direccionamientos in diccionario_codop[codop.upper()]:
            if(direccionamientos[2].upper() == 'INM'):
                codigoMaquinaTabop =  direccionamientos[3]
                ###
    ...
    elif(direccionamiento.upper() == 'INMEDIATO DE 16 BITS'):
        for direccionamientos in diccionario_codop[codop.upper()]:
            if(direccionamientos[2].upper() == 'INM'):
                codigoMaquinaTabop =  direccionamientos[3]
                ###
    ...
    elif(direccionamiento.upper() == 'INDIZADO DE 5 BITS'):
        for direccionamientos in diccionario_codop[codop.upper()]:
            if(direccionamientos[2].upper() == 'IDX'):
                codigoMaquinaTabop =  direccionamientos[3]
    ...
    elif(direccionamiento.upper() == 'INDIZADO DE 9 BITS'):
        for direccionamientos in diccionario_codop[codop.upper()]:
            if(direccionamientos[2].upper() == 'IDX1'):
                codigoMaquinaTabop =  direccionamientos[3]
                partes_operando = operando.split(',')
                valor = partes_operando[0]
                if(valor == ''): valor = '0'
                ...
                z ='0'
                if(int(valor) >= 0):
                    valor_hexadecimal = format(int(valor),'02X')
                    s = '0'
                else:
                    complemento_2 = int(valor) + 256
                    valor_hexadecimal = format(complemento_2,'02X')
                    s = '1'
    ...
    elif(direccionamiento.upper() == 'INDIZADO DE 16 BITS'):
        for direccionamientos in diccionario_codop[codop.upper()]:
            if(direccionamientos[2].upper() == 'IDX2'):
                codigoMaquinaTabop =  direccionamientos[3]
                partes_operando = operando.split(',')
                valor = partes_operando[0]
                if(valor == ''): valor = '0'
    ...
    elif(direccionamiento.upper() == 'INDIZADO DE PRE DECREMENTO'):
        for direccionamientos in diccionario_codop[codop.upper()]:
            if(direccionamientos[2].upper() == 'IDX'):
                codigoMaquinaTabop =  direccionamientos[3]
                partes_operando = operando.split(',')
    ...
    elif(direccionamiento.upper() == 'INDIZADO DE POST DECREMENTO'):
        for direccionamientos in diccionario_codop[codop.upper()]:
            if(direccionamientos[2].upper() == 'IDX'):
                codigoMaquinaTabop =  direccionamientos[3]
                partes_operando = operando.split(',')
    ...
    elif(direccionamiento.upper() == 'INDIZADO DE PRE INCREMENTO'):
        for direccionamientos in diccionario_codop[codop.upper()]:
            if(direccionamientos[2].upper() == 'IDX'):
                codigoMaquinaTabop =  direccionamientos[3]
                partes_operando = operando.split(',')
    ...
    elif(direccionamiento.upper() == 'INDIZADO DE POST INCREMENTO'):
        for direccionamientos in diccionario_codop[codop.upper()]:
            if(direccionamientos[2].upper() == 'IDX'):
                codigoMaquinaTabop =  direccionamientos[3]
                partes_operando = operando.split(',')
    ...
    elif(direccionamiento.upper() == 'INDIZADO DE ACUMULADOR'):
        for direccionamientos in diccionario_codop[codop.upper()]:
            if(direccionamientos[2].upper() == 'IDX'):
                codigoMaquinaTabop =  direccionamientos[3]
    ...
    elif(direccionamiento.upper() == 'INDIZADO INDIRECTO DE 16 BITS'):
        for direccionamientos in diccionario_codop[codop.upper()]:
            if(direccionamientos[2].upper() == '[IDX2]'):
                codigoMaquinaTabop =  direccionamientos[3]
    ...
    elif(direccionamiento.upper() == 'INDIZADO DE ACUMULADOR INDIRECTO'):
        for direccionamientos in diccionario_codop[codop.upper()]:
            if(direccionamientos[2].upper() == '[D,IDX]'):
                codigoMaquinaTabop =  direccionamientos[3]
    ...
    elif(direccionamiento.upper() == 'RELATIVO DE 8 BITS'):
        for direccionamientos in diccionario_codop[codop.upper()]:
            if(direccionamientos[2].upper() == 'REL'):
                codigoMaquinaTabop =  direccionamientos[3]
                #
                desplazamiento = 0
                valor_numerico = None
                ...
                for etiqueta in tabla_simbolos:
                    if(etiqueta[0] == operando):
                        valor_tabsim = etiqueta[1]
                        valor_numerico = int(valor_tabsim,16)
                        break
                valor_siguiente_localidad = int(archivo_listado[contador][0], 16)
                #print('Sig. Loc: '+archivo_listado[contador][0])
                if(valor_numerico != None):
                    desplazamiento = valor_numerico - valor_siguiente_localidad
                    #print('El desplazamiento es = ',desplazamiento, 'valor_numerico = ',valor_numerico,'valor_siguiente_localidad = ',valor_siguiente_localidad)
                    if(desplazamiento >= -128 and desplazamiento <= 127):
                        ...
                        codigoMaquinaCalculado  = 'Error (1)'
                        #print('\n\t\t     ERROR DE DESPLAZAMIENTO: El rango valido es de -128 a 127, y el desplazamiento es %d'%desplazamiento)
                else:
                    codigoMaquinaCalculado = 'Error (3)'
                    #print('\n\t\t     ERROR DE ETIQUETA: la etiqueta '+operando+' no existe en la tabla de simbolos')
                    
    elif(direccionamiento.upper() == 'RELATIVO DE 16 BITS'):
        for direccionamientos in diccionario_codop[codop.upper()]:
            if(direccionamientos[2].upper() == 'REL'):
                codigoMaquinaTabop =  direccionamientos[3]
                ...
                if(valor_numerico != None):
                    ...
                    if(desplazamiento >= -32768 and desplazamiento <= 32767):
                        ...
                        codigoMaquinaCalculado = 'Error (2)'
                        #print('\n\t\t     ERROR DE DESPLAZAMIENTO: El rango valido es de -32768 a 32767, y el desplazamiento es %d'%desplazamiento)
                else:
                    codigoMaquinaCalculado = 'Error (3)'
                    #print('\n\t\t     ERROR DE ETIQUETA: la etiqueta '+operando+' no existe en la tabla de simbolos')
                    
    elif(direccionamiento.upper() == 'DIRECTIVA DE CONSTANTE'):
        ...
        if(codop.upper() == 'FCC'):
            ...
            if(operando[0] == '$' or operando[0] == '@' or operando[0] == '%'):
                ...
                codigoMaquinaCalculado = format(int(valor), '02X')
            else:
            ...
            if(operando[0] == '$' or operando[0] == '@' or operando[0] == '%'):
                valor = validar_operando_directo_o_extendido(operando)
                codigoMaquinaCalculado = format(int(valor), '04X')
            else:
...
def direccionamiento_correspondiente(operando):
    ...
    if('INH' in direccionamientos_disponibles):
        if(operando == 'null' or operando == 'NULL'):
            ...
            resultado_evaluacion = 'Inherente'
    #-----------------------------------------------------------------------
    #                        INMEDIATO
    #-----------------------------------------------------------------------
    elif(operando[0] == '#'):
    ...
    #-----------------------------------------------------------------------
        ...
        if(operando[0] == '$' or operando[0] == '%' or operando[0] == '@' or (operando[0] >= '0' and operando[0] <= '9') or operando[0] == '-'):
            ...
            if('DIR' in direccionamientos_disponibles and (rango >= 0 and rango <= 255)):
                ...
                bytesTotales = int(totalBytes)
                resultado_evaluacion = 'Extendido'
            elif(rango > 65535):
            ...
            if(esValido):
                if('REL' in direccionamientos_disponibles):
                    ...
                    if(informacion[3] == '1'):
                        ...
                        bytesTotales = int(totalBytes)
                        resultado_evaluacion = 'Relativo de 8 bits'
                    elif(informacion[3] == '2'):
                        print('\t\t    Modo Relativo de 16 bits, %s bytes' %totalBytes)
                        bytesTotales = int(totalBytes)
                        resultado_evaluacion = 'Relativo de 16 bits'
                elif('EXT' in direccionamientos_disponibles):
                    informacion = lista_direccionamientos[2]
                    totalBytes = informacion[4]
                    bytesTotales = int(totalBytes)
                    print('\t\t    Modo Extendido, %s bytes' %totalBytes)
                    resultado_evaluacion = 'Extendido, Etiqueta'
    print()
                                ...
                                try:
                                    ...
                                    if(bytesPorCalcular == '1'):
                                        if(valor >= 0 and valor <= 255):
                                            ...
                                            resultado_evaluacion = 'Inmediato de 8 bits'
                                        else:
                                        ...
                                        if(valor >= 0 and valor <= 65535):
                                            ...
                                            resultado_evaluacion = 'Inmediato de 16 bits' 
                                        else:
        ...
        if(valor_valido and valor == 'D' or valor == 'd'):
                if(registro_valido):
                    if(error_sintaxis):
                        ...
                        resultado_evaluacion = 'Indizado de Acumulador Indirecto'
            ...
            if(valor_valido and registro_valido and not error_sintaxis):
                ...
                for datos in lista_direccionamientos:
                    if(datos[0] == '[IDX2]'):
                        informacion = datos
                        break
                ...
                resultado_evaluacion = 'Indizado Indirecto de 16 Bits' 
                bytesTotales = int(totalBytes)
            if(not valor_valido):
                ...
                if('+' in registro):
                    ...
                    if(posicion_signo[0] == ''):
                        if(valor_valido and registro_valido):
                            ...
                            resultado_evaluacion = 'Indizado de Pre Incremento'
                            bytesTotales = int(totalBytes)
                        else:
                            if(not valor_valido):
                                if(numero == None):
                        ...
                        if(valor_valido and registro_valido):
                            ...
                            resultado_evaluacion = 'Indizado de Post Incremento' 
                            bytesTotales = int(totalBytes)
                        else:
                            if(not valor_valido):
                                if(numero == None):
                    ...
                    if(posicion_signo[0] == ''):
                        if(valor_valido and registro_valido):
                            ...
                            resultado_evaluacion = 'Indizado de Pre Decremento'
                            bytesTotales = int(totalBytes)
                        else:
                            if(not valor_valido):
                                if(numero == None):
                        ...
                        if(valor_valido and registro_valido):
                            ...
                            resultado_evaluacion = 'Indizado de Post Decremento'
                            bytesTotales = int(totalBytes)
                        else:
                            if(not valor_valido):
                                if(numero == None):
            ...
            if(numero != None):
                    ...
                    if(numero >= -256 and numero <= -17):
                        if(registro_valido):
                            if(error_sintaxis):
                                ...
                                resultado_evaluacion = 'Indizado de 9 Bits'
                                bytesTotales = int(totalBytes)
                        else:
                        ...
                        if(registro_valido):
                            if(error_sintaxis):
                                ...
                                resultado_evaluacion = 'Indizado de 5 Bits'
                                bytesTotales = int(totalBytes)
                        else:
                        ...
                        if(registro_valido):
                            if(error_sintaxis):
                                ...
                                resultado_evaluacion = 'Indizado de 16 Bits'
                                bytesTotales = int(totalBytes)
                        else:
                            print('\t\t    ERROR DE REGISTRO: Los Registros validos para Modo Indizado de 16 Bits son X, Y, PC y SP')
                ...
                if(valor[0] == '@' or valor[0] == '$' or valor[0] == '%' or valor[0] == '-'):
                    ...
                    if(valor_valido and registro_valido):
                        ...
                        resultado_evaluacion = 'Indizado de Acumulador'
                        bytesTotales = int(totalBytes)
    return bytesTotales, resultado_evaluacion
...
def calcularCheckSum(calcular):
    ...
    #hexa = hex(int(complemento,2))[2:].upper()[2:]
    hexa = format(int(complemento,2), '04X')[2:]
    return hexa
